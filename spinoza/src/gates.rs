...
fn rz_apply_target(state_re: SendPtr<Float>, state_im: SendPtr<Float>, i: usize, m: &Amplitude) {
    unsafe {
        // a + ib
        let a = *state_re.get().add(i);
        let b = *state_im.get().add(i);

        *state_re.get().add(i) = a.mul_add(m.re, -b * m.im);
        *state_im.get().add(i) = a.mul_add(m.im, b * m.re);
    }
}
